...
public class DownloaderService(IEventBus eventBus, DownloaderServiceOptions serviceOptions)
    : IDownloaderService
{
    ...
    public async Task<IReadOnlyCollection<DownloadResult>> DownloadFileAsync(
        ...
        var outputPath = serviceOptions.OutputPath;
        ConcurrentBag<DownloadResult> results = [];
        var options = new ParallelOptions { CancellationToken = ct };
        using HttpClient client = new HttpClient { Timeout = TimeSpan.FromSeconds(serviceOptions.TimeoutSeconds) };
        await Parallel.ForEachAsync(addresses.Urls,
            options,
            async (address, token) =>
                {
                    try {
                        var result = await DownloadFileAsync(client, address, outputPath, token);
                        results.Add(result);
                    }
                    catch (Exception ex) {
    ...
    private async Task<DownloadResult> DownloadFileAsync(
        ...
        try {
            ...
            if (totalFileSize > serviceOptions.FileSizeMb * Mb)
                throw new FileSizeException($"Размер файла больше установленного лимита {serviceOptions.FileSizeMb}Mb");
